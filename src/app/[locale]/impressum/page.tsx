...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  const tMeta = await getTranslations({ locale, namespace: "metadata.impressum" });
  const canonical = getCanonicalUrl(locale, "/impressum");
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: getAlternateUrls("/impressum"),
    },
    openGraph: {
      title: tMeta("title"),
      description: tMeta("description"),
  ...
  const jsonLd = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Impressum", item: canonical },
      ],
    },
  ];
