...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  const tMeta = await getTranslations({ locale, namespace: "metadata.cookies" });
  const canonical = getCanonicalUrl(locale, "/cookie-policy");
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: getAlternateUrls("/cookie-policy"),
    },
    openGraph: {
      title: tMeta("title"),
      description: tMeta("description"),
...
export default async function CookiePolicyPage({ params: { locale } }: Props) {
