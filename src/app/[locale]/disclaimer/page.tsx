...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  const tMeta = await getTranslations({ locale, namespace: "metadata.disclaimer" });
  const canonical = getCanonicalUrl(locale, "/disclaimer");
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: getAlternateUrls("/disclaimer"),
    },
    openGraph: {
      title: tMeta("title"),
      description: tMeta("description"),
  ...
  const jsonLd = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { "@type": "ListItem", position: 2, name: "Disclaimer", item: canonical },
      ],
    },
  ];
